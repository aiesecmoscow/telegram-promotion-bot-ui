...
Если Telegram API недоступен напрямую (например, из РФ), задайте SOCKS5-прокси в `.env`:
...
- `SOCKS5_USERNAME` / `SOCKS5_PASSWORD` — опциональны; задаются **вместе**, иначе бэкенд упадёт на старте с ошибкой валидации.
- При заданном `SOCKS5_HOST` Telethon направляет через прокси **и авторизацию, и рассылку**. Резолв DNS тоже идёт через прокси (`rdns=true`), чтобы Telegram-эндпоинты не резолвились локально.
- Пустой `SOCKS5_HOST` — прямое подключение, как до изменений.
...
| `SOCKS5_HOST`   | string | Хост SOCKS5-прокси. Пусто — прямое подключение.          |
